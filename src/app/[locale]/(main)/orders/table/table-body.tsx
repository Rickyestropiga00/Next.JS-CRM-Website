...
export function OrdersTableBody({
  ...
  const translateProductType = (type: string) => {
    switch (type) {
      case 'Physical':
        return productTypesT('physical');

      case 'Digital':
        return productTypesT('digital');

      case 'Service':
        return productTypesT('service');

      case 'Subscription':
        return productTypesT('subscription');
